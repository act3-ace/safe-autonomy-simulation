...
def test_init_no_args():
    integrator = safe_autonomy_simulation.entities.integrator.PointMassIntegrator3d(
        name="integrator"
    )
    assert integrator.material is safe_autonomy_simulation.materials.BLACK
    assert integrator.name == "integrator"
    assert np.all(integrator.state == np.array([0, 0, 0, 0, 0, 0]))
...
@pytest.mark.parametrize(
    ...
    [
        ("integrator", np.array([1, 2, 3]), np.array([4, 5, 6]), 3, 4, 5, "Euler"),
        ("integrator", np.array([1, 2, 3]), np.array([4, 5, 6]), 3, 4, 5, "RK45"),
    ],
)
def test_init_with_args(
    ...
    assert np.all(integrator.x == position[0])
    assert np.all(integrator.y == position[1])
    assert np.all(integrator.z == position[2])
    assert np.all(integrator.x_dot == velocity[0])
    assert np.all(integrator.y_dot == velocity[1])
    assert np.all(integrator.z_dot == velocity[2])
    assert np.all(integrator.wx == np.array([0]))
    assert np.all(integrator.wy == np.array([0]))
    assert np.all(integrator.wz == np.array([0]))
    assert np.all(integrator.position == position)
    assert np.all(integrator.velocity == velocity)
    assert np.all(integrator.orientation == np.array([0, 0, 0, 1]))
    assert np.all(integrator.angular_velocity == np.array([0, 0, 0]))
...
def test_state():
    ...
    assert np.all(integrator.state == np.array([0, 0, 0, 0, 0, 0]))
    integrator.state = np.array([1, 2, 3, 4, 5, 6])
    assert np.all(integrator.state == np.array([1, 2, 3, 4, 5, 6]))
