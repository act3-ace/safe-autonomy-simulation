...
def test_init_no_args():
    integrator = safe_autonomy_simulation.entities.integrator.PointMassIntegrator2d(
        name="integrator"
    )
    assert integrator.material is safe_autonomy_simulation.materials.BLACK
    assert integrator.name == "integrator"
    assert np.all(integrator.state == np.array([0, 0, 0, 0]))
...
@pytest.mark.parametrize(
    ...
    [
        ("integrator", np.array([1, 2]), np.array([3, 4]), 3, 4, 5, "Euler"),
        ("integrator", np.array([1, 2]), np.array([3, 4]), 3, 4, 5, "RK45"),
    ],
)
def test_init_with_args(
    ...
    assert np.all(integrator.wy == np.array([0]))
    assert np.all(integrator.wz == np.array([0]))
    assert np.all(integrator.position == np.concatenate((position, np.array([0]))))
    assert np.all(integrator.velocity == np.concatenate((velocity, np.array([0]))))
    assert np.all(integrator.orientation == np.array([0, 0, 0, 1]))
    assert np.all(integrator.angular_velocity == np.array([0, 0, 0]))
...
def test_state():
    ...
    assert np.all(integrator.state == np.array([0, 0, 0, 0]))
    integrator.state = np.array([1, 2, 3, 4])
    assert np.all(integrator.state == np.array([1, 2, 3, 4]))
