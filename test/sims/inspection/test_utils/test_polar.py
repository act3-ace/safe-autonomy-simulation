...
def test_get_vec_az_elev(azimuth, elevation, expected):
    v = safe_autonomy_simulation.sims.inspection.utils.polar.get_vec_az_elev(azimuth=azimuth, elevation=elevation)
    assert np.allclose(v, expected)


def test_sample_az_elev():
    azimuth, elevation = safe_autonomy_simulation.sims.inspection.utils.polar.sample_az_elev()
