...
def use_jax():
    ...
    if "USE_JAX" in os.environ:
        assert os.environ["USE_JAX"] in ["0", "1"], f'invalid value {os.environ["USE_JAX"]} for environment variable USE_JAX. Must be "0" or "1"'
        result = bool(int(os.environ["USE_JAX"]))
        if result:
            try:
                import jax
            except ImportError:
                raise ImportError("Environment variable `USE_JAX = 1` but JAX is not found in environment. JAX can be installed via `pip install safe-autonomy-simulation[jax]`")
        return result
    else:
        return False
